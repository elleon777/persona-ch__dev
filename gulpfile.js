...
function writePugMixinsFile(cb) {
  ...
  allBlocksWithPugFiles.forEach(function (blockName) {
    pugMixins += `include ${dir.blocks.replace(
      dir.src,
      "../"
...
function generateSvgSprite(cb) {
  ...
  if (
    nth.config.alwaysAddBlocks.indexOf("sprite-svg") > -1 &&
    fileExist(spriteSvgPath)
  ) {
...
function generatePngSprite(cb) {
  ...
  if (
    nth.config.alwaysAddBlocks.indexOf("sprite-png") > -1 &&
    fileExist(spritePngPath)
  ) {
    ...
    let fileName =
      ...
      Math.random()
        ...
        .replace(/[^0-9]/g, "") +
      ".png";
    let spriteData = src(spritePngPath + "*.png").pipe(
      spritesmith({
        imgName: fileName,
    ...
    let cssStream = spriteData.css.pipe(dest(`${dir.blocks}sprite-png/`));
...
function compileSass() {
  ...
  if (buildLibrary)
    fileList.push(`${dir.blocks}blocks-library/blocks-library.scss`);
...
function buildJs() {
  ...
  if (buildLibrary)
    entryList[
      "blocks-library"
    ] = `./${dir.blocks}blocks-library/blocks-library.js`;
...
function serve() {
  ...
  watch([`${dir.src}pages/**/*.pug`], { delay: 100 })
    ...
    .on("unlink", function (path) {
      let filePathInBuildDir = path
        .replace(`${dir.src}pages/`, dir.build)
        .replace(".pug", ".html");
  ...
  watch(
    [`${dir.src}scss/**/*.scss`, `!${dir.src}scss/style.scss`],
  ...
  watch(
    [`${dir.src}js/**/*.js`, `!${dir.src}js/entry.js`, `${dir.blocks}**/*.js`],
    { events: ["all"], delay: 100 },
  ...
  watch(
    [`${dir.blocks}**/img/*.{jpg,jpeg,png,gif,svg,webp}`],
    { events: ["all"], delay: 100 },
  ...
  // Спрайт PNG
  watch(
    [`${dir.blocks}sprite-png/png/*.png`],
    { events: ["all"], delay: 100 },
    series(generatePngSprite, copyImg, compileSass, reload)
  );
}
...
function getClassesToBlocksList(file, enc, cb) {
  ...
  if (processThisFile) {
    ...
    console.log(
      "---------- Used HTML blocks: " + nth.blocksFromHtml.join(", ")
    );
...
function filterShowCode(text, options) {
  ...
  for (var i = 0; i < lines.length - 1; i++) {
    // (lines.length - 1) для срезания последней строки (пустая)
